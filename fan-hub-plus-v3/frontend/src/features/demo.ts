...
type StoredCampaign = Omit<Campaign, "entryCount" | "myEntry"> & {
  privateSeed: string;
  entries: Entry[];
};
type StoredReport = Pick<
  SocialData["reports"][number],
  "id" | "postId" | "commentId" | "userId" | "reason" | "resolved" | "createdAt" | "reporterName"
>;
interface State {
  schema: 3;
  social: Omit<SocialData, "reports"> & { reports: StoredReport[] };
...
function read(): State {
  try {
    ...
    if (s?.schema === 3 && s.social?.posts && Array.isArray(s.campaigns)) {
      s.social.posts = s.social.posts.map((p: any) => {
...
function write(s: State) {
  try {
    ...
    throw new Error(
      "Storage is unavailable or full. Your change was not saved.",
    );
  }
}
function actor(user: Author | null, admin = false): Author {
...
export async function sha256(s: string) {
  if (!crypto.subtle)
    throw new Error(
      "Secure browser cryptography is unavailable. Open this demo on localhost or HTTPS.",
    );
  return [
    ...new Uint8Array(
...
export async function rankTickets(
  ...
  const key = await crypto.subtle.importKey(
    "raw",
    new Uint8Array(seed.match(/../g)!.map((x) => parseInt(x, 16))),
      ...
      score: [
        ...new Uint8Array(
          await crypto.subtle.sign(
            "HMAC",
            key,
            new TextEncoder().encode(snapshotHash + "|" + ticket),
          ),
        ),
      ]
        .map((x) => x.toString(16).padStart(2, "0"))
  ...
  return rows
    .sort(
      (a, b) =>
        a.score.localeCompare(b.score) || a.ticket.localeCompare(b.ticket),
    )
    .map((x) => x.ticket);
}
...
function validPost(p: PostInput) {
  ...
  if (p.rating > 0) meaningfulBody(p.body);
  if (containsSensitive(p.title, p.subject, p.body, p.mediaUrl))
    throw new Error(
      "This post cannot include sensitive or explicit content. Remove it and try again.",
    );
...
export const demo = {
  async social(user: Author | null): Promise<SocialData> {
    const s = read().social;
    const visible = s.posts.filter(
      (p) =>
        p.status === "published" ||
        p.authorId === user?.id ||
        user?.role === "admin",
      ...
      user?.role === "admin"
        ? s.reports.map((r) => {
            ...
            return {
              ...
              authorName:
                comment?.authorName || post?.authorName || "Unknown",
              contentPreview: (comment?.body || post?.body || "").slice(
                0,
                280,
              ),
            };
  ...
  async removePost(user: Author | null, id: string) {
    const u = actor(user),
      s = read(),
      p = findPost(s, id, u);
  ...
  async moderate(
    ...
    p.status = decision as "published" | "rejected";
    p.reason = reason.slice(0, 500);
    p.version++;
    write(s);
  },
  async react(user: Author | null, id: string, kind: "like" | "heart" | null) {
    const u = actor(user),
      s = read();
    findPost(s, id, u, true);
    if (kind !== null && !["like", "heart"].includes(kind))
      throw new Error("Invalid reaction.");
    s.social.reactions = s.social.reactions.filter(
      (r) => !(r.postId === id && r.userId === u.id),
    );
    if (kind) s.social.reactions.push({ postId: id, userId: u.id, kind });
    write(s);
  },
  async comment(
    user: Author | null,
    ...
    if (
      parentId &&
      !s.social.comments.some(
        (c) => c.id === parentId && c.postId === id && !c.parentId && !c.hidden,
      )
    )
  ...
  async removeComment(user: Author | null, id: string) {
    const u = actor(user),
      s = read(),
      c = s.social.comments.find((c) => c.id === id);
  ...
  async resolveReport(user: Author | null, id: string, hide: boolean) {
    actor(user, true);
    const s = read(),
      r = s.social.reports.find((r) => r.id === id);
  ...
  async publicProfile(id: string) {
    const s = read().social;
    const post =
      s.posts.find((p) => p.authorId === id) ||
      s.comments.find((c) => c.authorId === id);
    if (!post) throw new Error("This member is not available.");
    return {
      id,
      name: (post as { authorName: string }).authorName,
      role: id === "u-admin" ? "admin" : "member",
      bio: "",
      favoriteCategories: [],
      createdAt: "",
      publishedPosts: s.posts.filter(
        (p) => p.authorId === id && p.status === "published",
      ).length,
    };
  },
  async campaigns() {
  ...
  async campaign(user: Author | null, id?: string): Promise<Campaign> {
    let s = read();
    const q = quarter();
    let c = s.campaigns.find((c) => c.id === (id || q.id));
    if (!c) {
      if (id && id !== q.id) throw new Error("Campaign not found.");
      const bytes = crypto.getRandomValues(new Uint8Array(32)),
        seed = [...bytes].map((x) => x.toString(16).padStart(2, "0")).join("");
  ...
  async enter(user: Author | null, id: string, agree: boolean) {
    const u = actor(user);
    if (u.role === "admin")
      throw new Error("Promoter accounts do not participate.");
    if (!agree) throw new Error("Read and accept the rules first.");
    const s = read(),
    ...
    if (c.entries.some((e) => e.userId === u.id)) return;
    const ticket =
      "FH-" + crypto.randomUUID().replace(/-/g, "").slice(0, 12).toUpperCase();
    c.entries.push({
      ticket,
      userId: u.id,
  ...
  async freeze(user: Author | null, id: string) {
    actor(user, true);
    let s = read(),
      c = s.campaigns.find((c) => c.id === id);
    if (!c || c.status !== "open")
    ...
    const tickets = c.entries.map((e) => e.ticket).sort(),
      hash = await sha256(tickets.join("\n"));
    s = read();
    c = s.campaigns.find((c) => c.id === id);
  ...
  async draw(user: Author | null, id: string) {
    actor(user, true);
    let s = read(),
      c = s.campaigns.find((c) => c.id === id);
    if (!c) throw new Error("Campaign not found.");
    if (c.status === "drawn") return;
    if (c.status !== "locked" || !c.snapshotHash)
      throw new Error("Lock the entry snapshot first.");
    const ordered = await rankTickets(
      c.privateSeed,
      c.snapshotHash,
      c.snapshot,
    );
  ...
  async chat(
    ...
    const q = normalize(question),
      stop = new Set(
        "the a an and or of to in is it what how about tell me please can you this that with for phim ve la cua mot nhung va cac hay tom tat giai thich co che".split(
          " ",
        ),
      );
    const words = [
    ...
    const candidates = knowledge
      ...
      .map((k) => {
        ...
        const exact = aliases.some((a) =>
          (" " + q + " ").includes(" " + a + " "),
        );
        const titles = normalize(k.title + " " + k.aliases.join(" ")).split(
          /\W+/,
        );
  ...
  async history(user: Author | null): Promise<Message[]> {
    return read().chats[user?.id || "visitor"] || [];
  },
  async saveChat(user: Author | null, messages: Message[]) {
    const s = read();
    s.chats[user?.id || "visitor"] = messages.slice(-40);
    write(s);
  },
  async clearChat(user: Author | null) {
    const s = read();
...
export async function verifyCampaignRecord(c: Campaign): Promise<boolean> {
  if (
    c.status !== "drawn" ||
    !c.seedReveal ||
    !c.snapshotHash ||
    ...
    return false;
  if (!/^[a-f0-9]{64}$/i.test(c.seedReveal)) return false;
  if (
    new Set(c.snapshot).size !== c.snapshot.length ||
    [...c.snapshot].sort().join("\n") !== c.snapshot.join("\n")
  )
    return false;
  if (
    (await sha256(c.seedReveal)) !== c.seedCommitment ||
    (await sha256(c.snapshot.join("\n"))) !== c.snapshotHash
  )
    return false;
  const ranked = await rankTickets(c.seedReveal, c.snapshotHash, c.snapshot),
    prizes = [...c.prizes].sort((a, b) => a.rank - b.rank),
    winners = [...c.winners].sort((a, b) => a.rank - b.rank);
  if (winners.length !== Math.min(ranked.length, prizes.length)) return false;
