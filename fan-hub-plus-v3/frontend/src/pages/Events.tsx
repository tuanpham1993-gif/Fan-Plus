...
export default function Events({ id }: { id?: string }) {
  ...
  const [view, setView] = useState("list"),
    ...
    [selected, setSelected] = useState("e1"),
    [month, setMonth] = useState(new Date(Date.UTC(2026, 9, 1)));
  const city = params.get("city") || "",
    radius = Number(params.get("radius") || 0);
  const events = useMemo(() => {
    const items = (db?.events || [])
      .filter((e) => !city || e.city === city)
      .map((e) => ({ ...e, distance: geo ? distanceKm(geo, e) : null }))
      .filter((e) => !geo || !radius || e.distance! <= radius);
    ...
    return (
      <>
        ...
        <div className="event-detail">
          <img
            src={event.image}
            alt="Original event concept artwork"
            width="1280"
            height="900"
          />
          <div>
            <span className="eyebrow">A FICTIONAL COMMUNITY EVENT</span>
            <h1>{event.title}</h1>
            ...
            <div className="event-detail-facts">
              ...
              <p>
                ...
                {date(event.endsAt, { timeStyle: "short" })} (Asia/Ho_Chi_Minh)
              </p>
              <p>
                <Icon name="pin" />
  ...
  return (
    <>
      ...
      <Notice>
        All events below are fictional test data. Location is requested only
        when you choose it, kept in memory and never saved. Online maps contact
        OpenStreetMap only after you load them.
      </Notice>
      {geoError && (
      ...
      <div className="event-toolbar">
        <div>
          ...
          >
            ...
            {[...new Set(db.events.map((e) => e.city))].map((c) => (
      ...
      {view === "list" &&
        (events.length ? (
          <div className="event-list">
            {events.map((e) => (
              <article className="event-card" key={e.id}>
                <img src={e.image} alt="" width="280" height="180" />
                <div className="event-date">
                  <strong>{date(e.startsAt, { day: "2-digit" })}</strong>
                  <span>{date(e.startsAt, { month: "short" })}</span>
                </div>
                <div className="event-card-body">
                  ...
                  <div className="event-meta">
                    ...
                    <span>
                      ...
                      {date(e.startsAt, { timeStyle: "short" })} GMT+7
                    </span>
                    {e.distance !== null && (
                      <span>{e.distance.toFixed(1)} km away</span>
                    )}
                  </div>
                </div>
                <Link
          ...
          <Empty
            icon="pin"
            title="No events in this orbit"
            description="Try another city or a larger distance radius."
          >
            <Button
              onClick={() => {
                setGeo(null);
                    ...
                    .filter(
                      (e) =>
                        new Intl.DateTimeFormat("en-CA", {
                          ...
                          day: "2-digit",
                          timeZone: "Asia/Ho_Chi_Minh",
                        }).format(new Date(e.startsAt)) === key,
                    )
                    .map((e) => (
          ...
          <div className="map-list">
            {events.map((e) => (
              <button
                ...
                className={mapEvent?.id === e.id ? "active" : ""}
              >
            ...
            {!mapEvent ? (
              ...
              <>
                ...
                <div className="map-caption">
                  <span>Fictional venue / {mapEvent.city}</span>
              ...
              <div className="map-consent">
                ...
                <p>
                  Load a live OpenStreetMap for the selected fictional event.
                  Your browser will contact an external map provider.
                </p>
