          ...
          <p key={i}>
            {bits.map((t, j) =>
              t.startsWith("**") ? (
                ...
                <React.Fragment key={j}>{t}</React.Fragment>
              ),
            )}
          </p>
        );
...
export default function Detail({ id }: { id: string }) {
  ...
  const content = db?.contents.find(
    (c) => c.id === id && (c.status === "published" || user?.role === "admin"),
  );
  useEffect(() => {
    setRevealed(false);
    setMediaError(false);
    if (user && content) void perform(() => repository.recordView(id));
  }, [id, user?.id]);
  if (!db || !content)
    return (
      <Empty
        title="This story is unavailable"
        description="It may have been removed or may not be published yet."
      >
  ...
  const category = db.categories.find((c) => c.id === content.categoryId),
    related = db.contents
      .filter(
        (c) =>
          c.id !== id &&
      ...
      .slice(0, 3),
    myRating =
      db.ratings.find((r) => r.userId === user?.id && r.contentId === id)
        ?.value || 0,
    ratings = db.ratings.filter((r) => r.contentId === id);
    ...
    <>
      ...
      <div className="detail-layout">
        <article>
            ...
            <div className="media-player">
              ...
              >
                <source
                  ...
                  onError={() => setMediaError(true)}
                  type={
                    content.mediaUrl?.split("?")[0].endsWith(".mp4")
                      ? "video/mp4"
                      : "video/webm"
                  }
                />
              ...
              <p className="caption">
                Original six-second animation. Captions available. No
                third-party footage.
              </p>
            </div>
          )}
          {content.type === "audio" && (
            <div className="audio-player">
              ...
              <div>
                ...
                <p className="caption">
                  Transcript: eight seconds of instrumental synthesized tones.
                  No speech.
                </p>
              </div>
            </div>
          ...
          {mediaError && (
            <Notice kind="error">
              The media could not load. Check that the local media files are
              included, then refresh.
            </Notice>
          )}
          {["gallery", "merchandise"].includes(content.type) && (
            <>
              ...
              <div className="gallery-grid">
                {galleryImages.map((src, i) => (
                  <button
                    ...
                    aria-label={`Open concept image ${i + 1}`}
                  >
                    <img
                      src={src}
                      alt={`Original concept study ${i + 1}`}
                      width="640"
                      height="450"
                    />
                    <span>
          ...
          <div className="tag-list">
            {content.tags.map((t) => (
              <span className="chip static-chip" key={t}>
                {t}
              </span>
            ))}
          </div>
              ...
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  onClick={async () => {
                    if (!user) {
                      navigate(
                        "/login?next=" + encodeURIComponent("/content/" + id),
                      );
                      return;
                    }
                    setRatingBusy(true);
                    await perform(
                      () => repository.rate(id, n),
        ...
        {gallery !== null && (
          <>
            ...
            <div className="modal-actions">
              ...
              <span>{gallery + 1} / 3</span>
              <Button onClick={() => setGallery((gallery + 1) % 3)}>
                Next
              </Button>
            </div>
          </>
