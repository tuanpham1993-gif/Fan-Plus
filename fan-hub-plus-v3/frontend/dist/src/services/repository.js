import { api, json, serverMode, clearCsrf } from "../features/http.js";
...
async function verifier(password, salt) {
    const raw = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(salt + password));
...
async function setPassword(email, password) {
    const records = credentials(), salt = crypto.randomUUID();
...
export const repository = {
    ...
    currentUser(db) {
        return (db.users.find((x) => x.id === sessionStorage.getItem(SESSION_KEY) && !x.suspended) || null);
    },
    async login(email, password) {
        if (serverMode) {
            const data = await api("/auth/login", json("POST", { email, password }));
            clearCsrf();
            const db = readDb();
            db.users = db.users.filter((u) => u.id !== data.user.id);
            db.users.push(data.user);
    ...
    async forgot(email) {
        if (serverMode)
            throw new AppError("Email recovery is not configured in the extension demo. Contact the local administrator.");
        await sleep();
        const db = readDb(), u = db.users.find((x) => x.email === email.trim().toLowerCase());
        if (!u)
            return null;
        const token = crypto.randomUUID();
        sessionStorage.setItem("fanhub.demo.reset", JSON.stringify({
            token,
            userId: u.id,
    ...
    async resetPassword(token, password) {
        if (serverMode)
            throw new AppError("Email recovery is not configured in the extension demo.");
        await sleep();
        let record;
        try {
    ...
    async deleteCategory(categoryId) {
        ...
        const db = readDb();
        actor(db, true);
        if (db.contents.some((c) => c.categoryId === categoryId) ||
    ...
    async saveEvent(event) {
        ...
        if (!Number.isFinite(Date.parse(event.startsAt)) ||
