...
export default function Admin() {
    ...
    const moderate = async (decision) => {
        if (!review)
            return;
        setBusy(true);
        if (await perform(() => repository.moderate(review.id, decision, reason), decision === "approved"
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement("div", { className: "admin-layout" },
            ...
            React.createElement("div", { className: "admin-content" }, section === "overview" ? (React.createElement(React.Fragment, null,
                ...
                React.createElement("section", { className: "panel" },
                    ...
                    React.createElement("div", { className: "analytics-bars" }, db.categories.map((cat) => {
                        const count = db.contents.filter((c) => c.categoryId === cat.id).length;
                        return (React.createElement("div", { key: cat.id },
                            ...
                            React.createElement("div", { className: "bar-track" },
                                React.createElement("div", { style: {
                                        width: (count /
                                            Math.max(1, ...db.categories.map((c) => db.contents.filter((x) => x.categoryId === c.id).length))) *
                                            100 +
                                            "%",
                ...
                React.createElement("div", { className: "two-column" },
                    React.createElement("section", { className: "panel" },
                        ...
                        React.createElement("p", null, pending.length
                ...
                section === "content" && (React.createElement("div", { className: "table-wrap" },
                    React.createElement("table", null,
                        ...
                        React.createElement("tbody", null, db.contents
                ...
                section === "categories" && (React.createElement("div", { className: "admin-card-grid" }, db.categories
                ...
                section === "events" && (React.createElement("div", { className: "admin-card-grid" }, db.events
                ...
                section === "submissions" && (React.createElement("div", { className: "stack" }, db.submissions.filter((s) => matches(s.title)).length ? (db.submissions
                    ...
                    React.createElement("table", null,
                        React.createElement("thead", null,
                            React.createElement("tr", null,
                                ...
                                React.createElement("th", { scope: "col" }, "Status"),
                                React.createElement("th", { scope: "col" }, "Actions"))),
                        React.createElement("tbody", null, db.users
                            ...
                            React.createElement("td", null,
                                React.createElement(Button, { variant: "secondary", disabled: u.id === user.id, onClick: () => remove((u.suspended ? "Enable " : "Suspend ") +
                                        u.name, () => perform(() => repository.setUserStatus(u.id, !u.suspended), "Demo account updated.")) }, u.suspended ? "Enable" : "Suspend"))))))))),
                section === "feedback" && (React.createElement("div", { className: "stack" }, db.feedback.filter((f) => matches(f.message)).length ? (db.feedback
                ...
                section === "knowledge" && (React.createElement("div", { className: "stack" }, db.faqs
        ...
        React.createElement(Modal, { open: !!editor, onClose: () => setEditor(null), title: editor?.value.id
                ? "Edit " + editor.kind
                : "Create " + (editor?.kind || "entry"), wide: true }, editor && (React.createElement(EditorialForm, { editor: editor, onDone: () => setEditor(null) }))),
        React.createElement(Confirm, { open: !!deletion, onClose: () => setDeletion(null), title: "Confirm this change", description: `You are about to change or remove: ${deletion?.title || ""}. This affects the local demo workspace. Destructive deletions cannot be undone except by resetting the demo.`, onConfirm: async () => {
                await deletion?.run();
            } }),
