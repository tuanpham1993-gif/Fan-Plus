import type { Content, Filter, Page, FanEvent, Database, User } from './types';
export const normalize = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
export function filterContents(contents: Content[], f: Filter, pageSize = 9): Page<Content> {
...
export function distanceKm(a: {
    ...
    const rad = (x: number) => x * Math.PI / 180, dlat = rad(b.lat - a.lat), dlng = rad(b.lng - a.lng);
...
export function safeReturnPath(value: string | null) { return value && value.startsWith('/') && !value.startsWith('//') && !value.includes('\\') ? value : '/dashboard'; }
export function recommend(db: Database, user: User | null, limit = 4) {
...
export function validateContent(c: Partial<Content>): string | null {
...
export function calendarFile(event: FanEvent): string {
    ...
    const dt = (x: string) => new Date(x).toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
    // Fold on UTF-8 byte boundaries as required by RFC 5545.
    const fold = (s: string) => { let out = '', line = '', size = 0; for (const ch of s) {
