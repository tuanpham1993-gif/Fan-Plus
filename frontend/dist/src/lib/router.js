...
export function configureNavigationPort(next) { port = next; }
...
export function Link({ to, onClick, children, ...props }) {
    return React.createElement("a", { href: to, ...props, onClick: e => {
            onClick?.(e);
            if (!e.defaultPrevented && e.button === 0 && !e.metaKey && !e.ctrlKey && !e.shiftKey && !e.altKey && (!props.target || props.target === '_self') && !props.download) {
                e.preventDefault();
                navigate(to);
            }
        } }, children);
