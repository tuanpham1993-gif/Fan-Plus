...
export default function Detail({ id }) {
    ...
    const category = db.categories.find(c => c.id === content.categoryId), related = db.contents.filter(c => c.id !== id && c.categoryId === content.categoryId && c.status === 'published').slice(0, 3), myRating = db.ratings.find(r => r.userId === user?.id && r.contentId === id)?.value || 0, ratings = db.ratings.filter(r => r.contentId === id);
    ...
    return React.createElement(React.Fragment, null,
        React.createElement(Crumbs, { items: [{ label: 'Explore', to: '/explore' }, { label: category?.name || 'Community', to: '/explore?category=' + content.categoryId }, { label: typeLabels[content.type] }] }),
        ...
        React.createElement("div", { className: "detail-layout" },
            React.createElement("article", null,
                React.createElement("div", { className: "detail-title" },
                    React.createElement("span", { className: "eyebrow" }, content.status === 'draft' ? 'UNPUBLISHED DRAFT - ADMIN PREVIEW' : 'FROM THE FAN HUB COLLECTION'),
                    ...
                    React.createElement("div", { className: "byline" },
                        ...
                        React.createElement("span", null,
                            ...
                            React.createElement("small", null,
                                new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium', timeZone: 'Asia/Ho_Chi_Minh' }).format(new Date(content.publishedAt)),
                                " / ",
                                content.duration)),
                ...
                content.type === 'video' && React.createElement("div", { className: "media-player" },
                    React.createElement("video", { key: id, controls: true, preload: "metadata", poster: content.image, onError: () => setMediaError(true) },
                        React.createElement("source", { src: content.mediaUrl, onError: () => setMediaError(true), type: content.mediaUrl?.split('?')[0].endsWith('.mp4') ? 'video/mp4' : 'video/webm' }),
                ...
                React.createElement("section", { className: "rating-panel" },
                    React.createElement("div", null,
                        ...
                        React.createElement("p", null, ratings.length ? `${ratings.length} demo rating${ratings.length > 1 ? 's' : ''} / ${(ratings.reduce((s, r) => s + r.value, 0) / ratings.length).toFixed(1)} average` : 'Be the first to rate this entry in your demo workspace.')),
