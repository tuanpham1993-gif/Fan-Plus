...
const nav = [['/dashboard', 'Overview'], ['/collection', 'My collection'], ['/profile', 'My profile'], ['/submit', 'My stories']];
function AccountNav() { return React.createElement("nav", { className: "account-nav", "aria-label": "Your workspace" }, nav.map(([to, label]) => React.createElement(Link, { key: to, to: to, className: currentPath() === to ? 'active' : '' }, label))); }
function Dashboard() {
    const { db, user } = useApp();
    if (!db || !user)
        return null;
    const saved = db.bookmarks.filter(b => b.userId === user.id), activity = db.activity.filter(a => a.userId === user.id).slice(0, 4), contributions = db.submissions.filter(s => s.userId === user.id), worlds = new Set(db.activity.filter(a => a.userId === user.id).map(a => db.contents.find(c => c.id === a.contentId)?.categoryId).filter(Boolean));
    return React.createElement(React.Fragment, null,
        ...
        React.createElement("div", { className: "stats-grid" },
            ...
            React.createElement("div", { className: "stat-card" },
                ...
                React.createElement("strong", null, contributions.filter(s => s.status === 'approved').length > 0 ? 'Contributor' : 'Explorer'),
                React.createElement("span", null, contributions.some(s => s.status === 'approved') ? 'A story of yours has been approved.' : 'Your discovery passport starts here.'))),
        ...
        React.createElement("div", { className: "two-column" },
            React.createElement("section", { className: "panel" },
                ...
                activity.length ? activity.map(a => { const c = db.contents.find(x => x.id === a.contentId); return c ? React.createElement(Link, { className: "activity-row", to: '/content/' + c.id, key: a.contentId },
                    ...
                    React.createElement(Icon, { name: "arrow", size: 16 })) : null; }) : React.createElement("p", { className: "muted" }, "Open a story, character or collectible to begin your reading history.")),
            React.createElement("section", { className: "panel" },
...
function Collection() {
    ...
    if (!db || !user)
        return null;
    const saved = db.bookmarks.filter(b => b.userId === user.id).map(b => ({ b, c: db.contents.find(c => c.id === b.contentId) })).filter(x => x.c?.title.toLowerCase().includes(q.toLowerCase()));
    return React.createElement(React.Fragment, null,
        ...
        saved.length ? React.createElement("div", { className: "card-grid" }, saved.map(({ b, c }) => c && React.createElement("div", { key: b.id },
            ...
            React.createElement("button", { className: "note-button", type: "button", onClick: () => { setNote(b); setText(b.note); } },
                React.createElement(Icon, { name: "edit", size: 15 }),
                b.note ? b.note : 'Add a private demo note')))) : React.createElement(Empty, { icon: "bookmark", title: q ? 'No matching saves' : 'Make room for your next favorite.', description: q ? 'Try another title or clear the search.' : 'Use the bookmark icon on any published discovery. Your collection is scoped to the signed-in demo profile.' },
...
function Profile() {
    ...
    const upload = async (file) => {
        ...
        try {
            ...
            bitmap.close();
            setAvatar(canvas.toDataURL('image/webp', .8));
        }
        catch {
    ...
    return React.createElement(React.Fragment, null,
        ...
        React.createElement("div", { className: "profile-grid" },
            React.createElement("form", { className: "panel stack-form", onSubmit: async (e) => { e.preventDefault(); setBusy(true); await perform(() => repository.updateProfile({ name, bio, favoriteCategories: cats, favoriteFandoms: fandoms.split(',').map(x => x.trim()).filter(Boolean), avatar }), 'Your profile has been updated.'); setBusy(false); } },
                ...
                React.createElement("fieldset", null,
                    ...
                    React.createElement("div", { className: "preference-grid" }, db.categories.map(c => React.createElement("label", { key: c.id, className: cats.includes(c.id) ? 'preference selected' : 'preference' },
                        React.createElement("input", { type: "checkbox", checked: cats.includes(c.id), onChange: () => setCats(s => s.includes(c.id) ? s.filter(x => x !== c.id) : [...s, c.id]) }),
            ...
            React.createElement("aside", { className: "stack" },
                ...
                React.createElement("section", { className: "panel" },
                    ...
                    React.createElement("div", { className: "stack" },
                        ...
                        React.createElement(Button, { variant: "secondary", onClick: async () => { setDb(await repository.logout()); navigate('/'); } },
                            "Sign out ",
        ...
        React.createElement(Confirm, { open: reset, onClose: () => setReset(false), title: "Reset this demo workspace?", description: "This removes all local profiles, notes, ratings, submissions, edits and chat history from Fan Hub Plus. Your appearance preference stays. Other websites are not affected.", onConfirm: async () => { setDb(repository.resetDemo()); notify('Demo workspace reset.'); navigate('/'); } }));
}
function Submit() {
    const { db, user, perform } = useApp();
    ...
    return React.createElement(React.Fragment, null,
        ...
        React.createElement("div", { className: "two-column submission-layout" },
            ...
            React.createElement("section", { className: "panel" },
                ...
                db.submissions.filter(s => s.userId === user.id).length ? db.submissions.filter(s => s.userId === user.id).map(s => React.createElement("article", { className: "submission-card", key: s.id },
        ...
        React.createElement("div", { className: "two-column" },
            ...
            React.createElement("section", { className: "panel" },
                ...
                user && React.createElement(React.Fragment, null,
                    ...
                    db?.feedback.filter(f => f.userId === user.id).slice(0, 4).map(f => React.createElement("div", { className: "submission-card", key: f.id },
                        React.createElement("span", { className: 'status status-' + f.status }, f.status),
                        React.createElement("p", null, f.message)))))));
}
export default function Account({ mode }) { return mode === 'dashboard' ? React.createElement(Dashboard, null) : mode === 'collection' ? React.createElement(Collection, null) : mode === 'profile' ? React.createElement(Profile, null) : mode === 'submit' ? React.createElement(Submit, null) : React.createElement(FeedbackPage, null); }
