...
export default function Explore({ mode = 'explore' }) {
    ...
    const title = mode === 'media' ? 'The media room' : mode === 'characters' ? 'The characters we connect with' : mode === 'showcase' ? 'Objects from other worlds' : 'Find your next obsession';
    const desc = mode === 'showcase' ? 'A curated collection of fictional collectibles. Made to discover, not to purchase.' : mode === 'media' ? 'A little motion, a little sound, and a lot of imagination.' : 'Eight categories. A thousand ways to get curious. Start with what you love.';
    const genres = [...new Set(db.contents.filter(c => c.status === 'published').map(c => c.genre))].sort(), fandoms = [...new Set(db.contents.filter(c => c.status === 'published').map(c => c.fandom))].sort();
    const active = Object.entries(filter).filter(([k, v]) => v && k !== 'page' && k !== 'sort' && !(mode !== 'explore' && k === 'type'));
    return React.createElement(React.Fragment, null,
        React.createElement(Crumbs, { items: [{ label: mode === 'explore' ? 'Explore' : mode === 'showcase' ? 'Showcase' : mode === 'media' ? 'Media room' : 'Characters' }] }),
        ...
        React.createElement("div", { className: "explore-search" },
            React.createElement("form", { onSubmit: e => { e.preventDefault(); update('q', q.trim()); }, className: "search-box" },
        ...
        React.createElement("div", { className: "category-tabs", role: "group", "aria-label": "Filter by category" },
            ...
            db.categories.map(c => React.createElement("button", { key: c.id, onClick: () => update('category', c.id), className: filter.category === c.id ? 'active' : '' }, c.name))),
            ...
            React.createElement("aside", { className: `filter-panel ${filtersOpen ? 'open' : ''}` },
                React.createElement("div", { className: "filter-panel-heading" },
                    React.createElement("h2", null,
                        ...
                        "Refine your world"),
                    React.createElement("button", { className: "small-link", onClick: () => { setQ(''); navigate(pathname); } }, "Reset")),
                ...
                ['explore', 'media'].includes(mode) && React.createElement("label", { className: "field" },
                    ...
                    React.createElement("select", { "aria-label": "Content type filter", value: params.get('type') || '', onChange: e => update('type', e.target.value) },
                        ...
                        Object.entries(typeLabels).filter(([k]) => mode !== 'media' || ['video', 'audio', 'gallery'].includes(k)).map(([k, v]) => React.createElement("option", { key: k, value: k }, v)))),
