...
const Assistant = React.lazy(() => import('./pages/Assistant.js'));
const Utility = React.lazy(() => import('./pages/Utility.js'));
const Releases = React.lazy(() => import('./pages/Releases.js'));
export class ErrorBoundary extends React.Component {
    state = { failed: false };
    static getDerivedStateFromError() { return { failed: true }; }
    componentDidCatch(error) { console.error('Fan Hub UI error:', error); }
    render() { return this.state.failed ? React.createElement("div", { className: "main-container" },
...
export default function App() {
    ...
    useEffect(() => { const names = { '/': 'Discover', '/explore': 'Explore', '/events': 'Events', '/showcase': 'Showcase', '/dashboard': 'Your workspace', '/collection': 'Your collection', '/admin': 'Editorial workspace' }; document.title = (names[pathname] || 'Explore your worlds') + ' | Fan Hub Plus'; }, [pathname]);
    let page;
    if (loading)
        ...
        const modes = { '/login': 'login', '/register': 'register', '/forgot-password': 'forgot', '/reset-password': 'reset', '/verify-email': 'verify' };
        ...
        page = React.createElement(Utility, { mode: pathname === '/sitemap' ? 'sitemap' : pathname === '/privacy' ? 'privacy' : '404' });
